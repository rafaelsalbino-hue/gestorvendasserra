...
import { FUNCOES_RESPONSAVEL, type FuncaoResponsavel, type Responsavel } from "@/types/contracts";
...
const Responsaveis = () => {
  ...
  const grouped = FUNCOES_RESPONSAVEL.reduce((acc, f) => {
    ...
    return acc;
  }, {} as Record<FuncaoResponsavel, Responsavel[]>);
    ...
    <AppLayout>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground">
              Cadastre as pessoas responsáveis por cada etapa do fluxo
            </p>
          </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {FUNCOES_RESPONSAVEL.filter(
            (f) => filterFuncao === "todas" || f === filterFuncao
          ).map((funcaoKey) => (
            <Card key={funcaoKey}>
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-base">{funcaoKey}</CardTitle>
                  <Badge className={funcaoColors[funcaoKey] + " text-xs"}>
                    {grouped[funcaoKey]?.length || 0}
                  </Badge>
                </div>
              </CardHeader>
              <CardContent>
