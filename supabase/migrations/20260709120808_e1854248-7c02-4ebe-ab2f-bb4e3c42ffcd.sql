...
-- Tarefa 2: coluna para registrar rota escolhida no avanço a partir do CRM
ALTER TABLE public.contratos_historico
  ADD COLUMN IF NOT EXISTS rota_escolhida text;

-- Tarefa 3: rota das permissões de notificação (padrao | crm_direto)
ALTER TABLE public.notificacao_permissoes
  ADD COLUMN IF NOT EXISTS rota text NOT NULL DEFAULT 'padrao';

CREATE INDEX IF NOT EXISTS idx_notif_perm_rota
  ON public.notificacao_permissoes (rota, entidade, canal, etapa);

-- Semeia registros da rota crm_direto (etapa proposta) copiando os da rota padrão
INSERT INTO public.notificacao_permissoes (entidade, canal, etapa, funcao, ativo, rota)
SELECT entidade, canal, etapa, funcao, false, 'crm_direto'
FROM public.notificacao_permissoes
WHERE rota = 'padrao' AND etapa = 'proposta'
ON CONFLICT DO NOTHING;

-- Atualiza trigger para escolher permissões pela rota quando OLD=crm e NEW=proposta
...
BEGIN
  ...
  IF OLD.etapa_atual IS DISTINCT FROM NEW.etapa_atual THEN
    ...
    -- Rota: crm_direto se veio de crm direto para proposta (pulou supervisor)
    IF OLD.etapa_atual::text = 'crm' AND NEW.etapa_atual::text = 'proposta' THEN
      v_rota := 'crm_direto';
    ELSE
      v_rota := 'padrao';
    END IF;
    ...
    JOIN public.notificacao_permissoes p
     ...
     AND p.canal = 'sistema' AND p.ativo = true
     AND p.entidade = v_entidade_perm
     AND p.rota = v_rota
    ...
    IF NEW.agente_pj_id IS NOT NULL THEN
      SELECT EXISTS (
        ...
        WHERE etapa = NEW.etapa_atual AND canal = 'sistema' AND ativo = true
          AND entidade = v_entidade_perm AND rota = v_rota
          AND funcao::text = 'Agente de Mercado PJ'
      ) INTO v_agente_ativa;
